...
/** Fields used by ChecklistLayout beyond the shared BaseTopicProps. */
interface ChecklistTopic extends BaseTopicProps {
  approved?: ChecklistItem[];
  awareness?: ChecklistItem[];
  forbidden?: ChecklistItem[];
}
...
export function ChecklistLayout({ topic, onBack }: LayoutProps) {
  // Cast once at entry; family-specific fields are typed via ChecklistTopic.
  const t = topic as ChecklistTopic;
      ...
      <div style={{ position: "relative", zIndex: 2, maxWidth: 1100, margin: "0 auto", padding: `${viewport.pagePaddingTop}px ${viewport.pagePaddingX}px ${viewport.pagePaddingBottom}px` }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: viewport.isCompact ? "1fr" : "1fr 1fr", gap: viewport.cardGap }}>
          <div>
            ...
            {(t.approved ?? []).map((item, i) => <Item key={i} item={item} delay={0.15 + i * 0.06} />)}
            {t.awareness && (
              <>
                ...
                {t.awareness.map((item, i) => <Item key={`a${i}`} item={item} delay={0.5 + i * 0.06} />)}
              </>
            )}
          </div>
          <div>
            ...
            {(t.forbidden ?? []).map((item, i) => <Item key={i} item={item} delay={0.2 + i * 0.06} />)}
          </div>
        </div>
