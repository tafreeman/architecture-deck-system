...
/** Fields used by OpFlowLayout beyond the shared BaseTopicProps. */
interface OpFlowTopic extends BaseTopicProps {
...
export function OpFlowLayout({ topic, onBack }: LayoutProps) {
  // Cast once at entry; family-specific fields are typed via OpFlowTopic.
  const top = topic as OpFlowTopic;
  ...
  return (
    <div style={{ position: "relative", minHeight: "100vh", background: T.bg, overflow: "hidden" }}>
      ...
      <div style={{ position: "absolute", top: 0, right: 0, width: "40%", height: "50%", background: `radial-gradient(ellipse at top right,${top.color}08,transparent 65%)`, pointerEvents: "none" }} />
      <div style={{ position: "relative", zIndex: 2, maxWidth: 1200, margin: "0 auto", padding: "24px 32px" }}>
        ...
        <div style={{ ...fade(0.05), display: "grid", gridTemplateColumns: "1fr 300px", gap: 24, marginBottom: 18 }}>
          <div>
            ...
            <p style={{ fontSize: 12, color: T.textMuted, lineHeight: 1.65, margin: 0, maxWidth: 540 }}>{top.subheadline || top.headline || ""}</p>
          </div>
          {/* Snapshot */}
            ...
            {[row1, row2].filter(row => row.length > 0).map((row, ri) => (
              <div key={ri} style={{ ...fade(0.18 + ri * 0.08), display: "grid", gridTemplateColumns: `repeat(${row.length},1fr)`, gap: 8, marginBottom: 8 }}>
                {row.map((step, i) => {
                  ...
                  return (
                    <div key={i} style={{ background: T.bgCard, borderRadius: C.innerRadius, padding: "11px 13px", borderTop: `2px solid ${isAI ? top.color : top.color + "38"}` }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 5 }}>
                        <span style={{ fontFamily: T.fontDisplay, fontSize: 13, fontWeight: C.headingWeight, color: isAI ? top.color : T.textDim }}>{step.num as string}</span>
                        <span style={{ fontSize: 7.5, padding: "1px 6px", borderRadius: 3, background: isAI ? top.color + "20" : "rgba(255,255,255,0.05)", color: isAI ? top.color : T.textDim, fontFamily: T.fontDisplay, fontWeight: C.headingWeight, letterSpacing: 0.5 }}>{isAI ? UI.WORKFLOW.ai : UI.WORKFLOW.human}</span>
                      </div>
        ...
        <div style={{ ...fade(0.34), display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
          ...
          <div style={{ background: top.color + "0F", borderRadius: C.cardRadius, padding: "13px 16px", border: `1px solid ${top.color}22`, borderLeft: `3px solid ${top.color}` }}>
